...
def transaction(sender_account, receiver_account, amount):
    if sender_account in database and receiver_account in database:
        if database[sender_account]['Balance'] >= amount:
            ...
            save_trans(sender_account, database[sender_account]['First Name'], database[sender_account]['Last Name'], amount, "Sent")
            save_trans(receiver_account, database[receiver_account]['First Name'], database[receiver_account]['Last Name'], amount, "Received")
        else:
...
def save_trans(account_number, first_name, last_name, amount, transaction_type):
    with open("transactions.txt", "a") as file:
        file.write(f"{first_name}, {last_name}, {account_number}, {amount}, {transaction_type}\n")
